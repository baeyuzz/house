...
public class HouseController {
	...
	private ResponseEntity<Map<String, Object>> pageNav(@RequestBody Map<String, Object> param, HttpSession session) {
		...
		// 페이지 네비게이션 만들기
		PageNavigation pageNavigation = PageNavigation.makePageNavigation(currentPage, HousePageBean.INTERVAL, n);
	...
	@ResponseBody
	@PostMapping("/sort")
	private ResponseEntity<Map<String, Object>> houseSort(@RequestBody Map<String, Object> param, HttpSession session) {
	...
	private ResponseEntity<Map<String, Object>> houseDetail(@PathVariable int no, HttpSession session) {
		...
		String addr = addrArr[1] + addrArr[2] + house.getAptName();
	...
	public ResponseEntity<Map<String, Object>> getChartData(@RequestBody Map<String, Object> param) {
		...
		List<Integer> deposit = new ArrayList<Integer>();
		...
		for (HouseDeal deal : list) {
			if (deal.getType() == 1 || deal.getType() == 2) {
				...
				monthly.add(deal.getDealDate());
				deposit.add(deal.getDealAmount());
