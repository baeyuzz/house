					...
					<div class="form-group">
						<div class="form-check-inline">
							<label class="form-check-label">
								아파트 매매
								<input type="checkbox" v-model="aptdeal" class="form-check-input" name="aptdeal"/>
							</label> 
						</div>
						<div class="form-check-inline">
							<label class="form-check-label">
								연립,다세대 매매
								<input type="checkbox" v-model="housedeal" class="form-check-input" name="housedeal"/>
							</label>
						</div>
						<div class="form-check-inline">
							<label class="form-check-label">
								아파트 전월세
								<input type="checkbox" v-model="aptrent" class="form-check-input" name="aptrent"/>
							</label> 
						</div>
						<div class="form-check-inline">
							<label class="form-check-label">
								연립,다세대 전월세
								<input type="checkbox" v-model="houserent" class="form-check-input" name="houserent"/>
							</label>
						</div>					
					</div>
					<div class="form-group form-inline sms-form-center">
						<label for="dong" class="mr-sm-2">동 이름:</label>
						<input v-model="dong" type="text" class="form-control mr-sm-2" 
							name="dong" placeholder="동 이름을 입력해주세요"/>
						<label for="aptname" class="mr-sm-2">아파트 이름:</label>
						<input v-model="aptname" type="text" class="form-control mr-sm-2" 
							name="aptname" placeholder="아파트 이름을 입력해주세요"/>
    ...
    export default {
        ...
        methods: {
            ...
            , search() {
                ...
                // 아무것도 선택 안하면 검색 안됨!
                if(!this.aptdeal && !this.housedeal 
                && !this.aptrent && !this.houserent) {
                    alert('최소한 하나의 타입은 선택해야합니다.');
                    return;
                }

                http.post('/house/list', {
